...
from know.core import article_markdown, permissions
...
class Article(models.Model):
    ...
    current_revision = models.OneToOneField(
        ...
        help_text=_(u'The revision being displayed for this article. \
            If you need to do a roll-back, simply change the value of \
            this field.'),
    )
    ...
    owner = models.ForeignKey(
        ...
        help_text=_(u'The owner of the article, usually the creator. \
    ...
    group = models.ForeignKey(
        ...
        blank=True,
        null=True,
        help_text=_(u'Like in a UNIX file system, permissions can \
            be given to a user according to group membership. Groups \
            are handled through the Django auth system.'),
        on_delete=models.SET_NULL)
    ...
    def add_object_relation(self, obj):
        ...
        rel = ArticleForObject.objects.get_or_create(
            article=self,
    ...
    def get_cache_key(self):
        return "know:article:%d" % (self.current_revision.id if self.current_revision else self.id)
...
class ArticleRevision(BaseRevisionMixin, models.Model):
    ...
    content = models.TextField(
        blank=True,
        verbose_name=_(u'article contents'),
    )
    # This title is automatically set from either the article's title or
    # the last used revision...
    title = models.CharField(
        max_length=512,
        verbose_name=_(u'article title'),
        null=False,
        blank=False,
        help_text=_(u'Each revision contains a title field that must be \
            filled out, even if the title has not changed'),
    )
