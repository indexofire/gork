...
def response_forbidden(request, article, urlpath):
    if request.user.is_anonymous():
        ...
        c = RequestContext(request, {'article': article, 'urlpath': urlpath})
        return HttpResponseForbidden(render_to_string("know/permission_denied.html", context_instance=c))
...
def get_article(func=None, can_read=True, can_write=False,
    ...
    can_delete and can_moderate: Verifies with know.core.permissions
    ...
    Also see: know.views.mixins.ArticleMixin
    """
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not path is None:
            try:
                ...
                return redirect('know:root_create')
                from .views import article
                #from feincms.content.application.models import app_reverse
                #return app_reverse('root_create', 'know.urls', (), {})
                ...
                try:
                    ...
                    return redirect(reverse("know:create", kwargs={'path': parent.path, }) + "?slug=%s" % pathlist[-1])
                except models.URLPath.DoesNotExist:
                    c = RequestContext(request, {'error_type': 'ancestors_missing'})
                    return HttpResponseNotFound(render_to_string("know/error.html", context_instance=c))
            if urlpath.article:
                ...
                return_url = reverse('know:get', kwargs={'path': urlpath.parent.path})
                urlpath.delete()
        ...
        if not deleted_contents:
            ...
            if urlpath:
                if urlpath.is_deleted():  # This also checks all ancestors
                    return redirect('know:deleted', path=urlpath.path)
            else:
                if article.current_revision and article.current_revision.deleted:
                    return redirect('know:deleted', article_id=article.id)
