...
from know import editors, forms, models
from know.conf import settings
from know.core.plugins import registry as plugin_registry
from know.core.diff import simple_merge
from know.decorators import get_article, json_view
from know.core.exceptions import NoRootURL
from know.core import permissions
...
class Create(FormView, ArticleMixin):
    """
    Wiki create new page view
    """
    form_class = forms.CreateForm
    template_name = "know/create.html"
    ...
    def get_success_url(self):
        return redirect('know:get', self.newpath.path)
...
class Delete(FormView, ArticleMixin):
    """
    Wiki page delete view
    """
    form_class = forms.DeleteForm
    template_name = "know/delete.html"
    ...
    def dispatch1(self, request, article, *args, **kwargs):
        ...
        if urlpath and urlpath.parent:
            self.next = reverse('know:get', kwargs={'path': urlpath.parent.path})
        elif urlpath:
            ...
            for art_obj in article.articleforobject_set.filter(is_mptt=True):
                if art_obj.content_object.parent:
                    self.next = reverse('know:get', kwargs={'article_id': art_obj.content_object.parent.article.id})
                else:
                    self.cannot_delete_root = True
    ...
    def form_valid(self, form):
        ...
        if self.cannot_delete_root or cannot_delete_children:
            ...
            return redirect('know:get', article_id=self.article.id)
...
class Edit(FormView, ArticleMixin):
            ...
            if Form:
                if form_id == self.request.GET.get('f', None):
                    ...
                    if form.is_valid():
                        ...
                        if self.urlpath:
                            return redirect('know:edit', path=self.urlpath.path)
                        return redirect('know:edit', article_id=self.article.id)
    ...
    def get_success_url(self):
        ...
        if self.urlpath:
            return redirect("know:get", path=self.urlpath.path)
        return redirect('know:get', article_id=self.article.id)
...
class Deleted(Delete):
    ...
    template_name = "know/deleted.html"
    form_class = forms.DeleteForm
    ...
    def dispatch(self, request, article, *args, **kwargs):
        ...
        if self.urlpath:
            ...
            if deleted_ancestor is None:
                ...
                return redirect('know:deleted', path=deleted_ancestor.path)

        else:
            if not article.current_revision.deleted:
                return redirect('know:get', article_id=article.id)
...
class Source(ArticleMixin, TemplateView):
    """
    Check page source view
    """
    template_name = "know/source.html"
...
class History(ListView, ArticleMixin):
    """
    Check page history view
    """
    template_name = "know/history.html"
    allow_empty = True
...
class Dir(ListView, ArticleMixin):
    """
    List page directory view
    """
    template_name = "know/dir.html"
    allow_empty = True
...
class SearchView(ListView):
    ...
    Search View
    """
    template_name = "know/search.html"
    paginate_by = 25
...
class Settings(ArticleMixin, TemplateView):
    """
    Frontend wiki settings view
    """
    permission_form_class = forms.PermissionsForm
    template_name = "know/settings.html"
            ...
            if Form.action == self.request.GET.get('f', None):
                ...
                if form.is_valid():
                    ...
                    if usermessage:
                        messages.success(self.request, usermessage)
                    if self.urlpath:
                        return redirect('know:settings', path=self.urlpath.path)
                    return redirect('know:settings', article_id=self.article.id)
            else:
                form = Form(self.article, self.request)
    ...
    def get(self, *args, **kwargs):
        ...
        # There is a bug where articles fetched with select_related have
        # bad boolean field https://code.djangoproject.com/ticket/15040
...
def change_revision(request, article, revision_id=None, urlpath=None):
    ...
    article.save()
    messages.success(request, _(u"The article %(title)s is now set to \
        display revision #%(revision_number)d") % {
        'title': revision.title,
        'revision_number': revision.revision_number,
    })

    if urlpath:
        return redirect("know:history", path=urlpath.path)
    else:
        return redirect('know:history', article_id=article.id)
...
class Preview(ArticleMixin, TemplateView):
    ...
    Wiki page preview
    """
    template_name = "know/preview_inline.html"
...
def merge(request, article, revision_id, urlpath=None, template_file="know/preview_inline.html", preview=False):
    ...
    if not preview:
        ...
        if revision.deleted:
            c = RequestContext(request, {'error_msg': _(u'You cannot merge with a deleted revision'),
                                         'article': article,
                                         'urlpath': urlpath})
            return render_to_response("know/error.html", context_instance=c)
        ...
        messages.success(request, _(u'A new revision was created: Merge between revision #%(r1)d and revision #%(r2)d') %
                         {'r1': revision.revision_number,
                          'r2': old_revision.revision_number})
        if urlpath:
            return redirect('know:edit', path=urlpath.path)
        else:
            return redirect('know:edit', article_id=article.id)
...
def root_create(request):
    try:
        ...
        if not root.article:
            ...
            root.delete()
            raise NoRootURL
        return redirect('know:get', path=root.path)
    except NoRootURL:
        pass
    if not request.user.is_superuser:
        return redirect(settings.LOGIN_URL + "?next=" + reverse("know:root_create"))
    if request.method == 'POST':
        ...
        if create_form.is_valid():
            models.URLPath.create_root(title=create_form.cleaned_data["title"],
                                       ...
                                       request=request)
            return redirect("know:root")
    else:
        create_form = forms.CreateRootForm()

    c = RequestContext(request, {'create_form': create_form, 'editor': editors.getEditor(), })
    return render_to_response("know/create_root.html", context_instance=c)
