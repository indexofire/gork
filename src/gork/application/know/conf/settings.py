...
EDITOR = getattr(django_settings, 'WIKI_EDITOR', 'know.editors.markitup.MarkItUp')
...
CAN_ASSIGN = getattr(django_settings, 'WIKI_CAN_ASSIGN', lambda article, user: user.has_perm('know.assign'))
...
CAN_CHANGE_PERMISSIONS = getattr(django_settings, 'WIKI_CAN_CHANGE_PERMISSIONS', lambda article, user: article.owner == user or user.has_perm('know.assign'))
...
CAN_MODERATE = getattr(django_settings, 'WIKI_CAN_MODERATE', lambda article, user: user.has_perm('know.moderate'))

# A function returning True/False if a user has permission to create
# new groups and users for the know.
CAN_ADMIN = getattr(django_settings, 'WIKI_CAN_ADMIN', lambda article, user: user.has_perm('know.admin'))
...
if ACCOUNT_HANDLING:
    LOGIN_URL = reverse_lazy("know:login")
    LOGOUT_URL = reverse_lazy("know:logout")
    SIGNUP_URL = reverse_lazy("know:signup")
else:
...
#: dottedname of class used to construct urlpatterns for know.
#:
#: Default is know.urls.WikiURLPatterns. To customize urls or view handlers,
#: you can derive from this.
URL_CONFIG_CLASS = getattr(django_settings, 'WIKI_URL_CONFIG_CLASS', 'know.urls.WikiURLPatterns')
