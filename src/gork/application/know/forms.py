...
from know import models
from know.conf import settings
from know.editors import getEditor
from know.core.diff import simple_merge
from know.core.plugins.base import PluginSettingsFormMixin
    ...
    def check_spam(self):
        ...
        def check_interval(from_time, max_count, interval_name):
            ...
            if revisions >= max_count:
                raise forms.ValidationError(_(u'Spam protection: You are only allowed \
                    to create or edit %(revisions)d article per %(interval_name)s.') % {
                    'revisions': max_count, 'interval_name': interval_name, })

        if not settings.LOG_IPS_ANONYMOUS:
            return
        if request.user.has_perm('know.moderator'):
            return
        ...
        check_interval(from_time, per_minute, _('minute') if settings.REVISIONS_MINUTES_LOOKBACK == 1 else (_(u'%d minutes') % settings.REVISIONS_MINUTES_LOOKBACK), )
...
class CreateRootForm(forms.Form):
    ...
    content = forms.CharField(
        label=_(u'Type in some contents'),
...
class EditForm(forms.Form, SpamProtectionMixin):
    ...
    summary = forms.CharField(
        label=_(u'Summary'),
        help_text=_(u'Give a short reason for your edit, which will be stated in the revision log.'),
        required=False,
    )
...
class CreateForm(forms.Form, SpamProtectionMixin):
    ...
    slug = forms.SlugField(
        label=_(u'Slug'),
        help_text=_(u"This will be the address where your \
            article can be found. Use only alphanumeric \
            characters and - or _. Note that you cannot \
            change the slug after creating the article."),
...
class DeleteForm(forms.Form):
    ...
    purge = forms.BooleanField(
        ...
        required=False,
        label=_(u'Purge'),
        help_text=_(u'Purge the article: Completely remove it \
            (and all its contents) with no undo. Purging is a \
            good idea if you want to free the slug such that \
            users can create new articles in its place.'),
    )
...
class PermissionsForm(PluginSettingsFormMixin, forms.ModelForm):
    ...
    locked = forms.BooleanField(
        label=_(u'Lock article'),
        help_text=_(u'Deny all users access to edit this article.'),
        required=False,
    )
    ...
    recursive = forms.BooleanField(
        label=_(u'Inherit permissions'),
        help_text=_(u'Check here to apply the above permissions \
            (excluding group and owner of the article) recursively \
            to articles below this one.'),
        required=False,
    )

    recursive_owner = forms.BooleanField(
        label=_(u'Inherit owner'),
        help_text=_(u'Check here to apply the ownership setting \
            recursively to articles below this one.'),
        required=False,
    )

    recursive_group = forms.BooleanField(
        label=_(u'Inherit group'),
        help_text=_(u'Check here to apply the group setting \
            recursively to articles below this one.'),
        required=False,
    )
