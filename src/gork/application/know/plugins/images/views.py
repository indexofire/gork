...
from know.conf import settings as wiki_settings
from know.decorators import get_article
from know.plugins.images import forms
from know.plugins.images import models
from know.views.mixins import ArticleMixin


class ImageView(ArticleMixin, ListView):

    template_name = 'know/plugins/images/index.html'
    allow_empty = True
...
class DeleteView(ArticleMixin, RedirectView):
    ...
    def get_redirect_url(self, **kwargs):
        ...
        if self.urlpath:
            return reverse('know:images_index', kwargs={'path': self.urlpath.path})
        return reverse('know:images_index', kwargs={'article_id': self.article.id})


class PurgeView(ArticleMixin, FormView):

    template_name = "know/plugins/images/purge.html"
    permanent = False
    ...
    def form_valid(self, form):
        ...
        if self.urlpath:
            return redirect('know:images_index', path=self.urlpath.path)
        return redirect('know:images_index', article_id=self.article_id)
...
class RevisionAddView(ArticleMixin, FormView):
    ...
    template_name = "know/plugins/images/revision_add.html"
