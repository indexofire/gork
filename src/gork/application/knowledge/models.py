...
from know.managers import QuestionManager, ResponseManager
from know.signals import knowledge_post_save
...
class Question(KnowledgeBase):
    ...
    def get_absolute_url(self):
        ...
        if settings.SLUG_URLS:
            return ('knowledge_thread', 'know.urls', (), {'id': self.id, 'slug': slugify(self.title)})
            #return ('knowledge_thread', [self.id, slugify(self.title)])
        else:
            return ('knowledge_thread_no_slug', 'know.urls', (), {'id': self.id})
    ...
    def accepted(self):
        """ Returns a boolean indictating whether there is a accepted answer or not."""
