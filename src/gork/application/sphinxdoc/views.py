...
def documentation(request, slug, path):
    """
    Displays the contents of a :class:`sphinxdoc.models.SphinxDoc`.
    ...
    """
    project = get_object_or_404(SphinxDocProject, slug=slug)
    path = path.rstrip('/')

    try:
        index = 'index' if path == '' else '%s/index' % path
        doc = SphinxDoc.objects.get(project=project, path=index)
    except ObjectDoesNotExist:
        doc = get_object_or_404(SphinxDoc, project=project, path=path)
    ...
    return render_to_response(templates, data, context_instance=RequestContext(request))
