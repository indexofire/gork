...
class Command(BaseCommand):
    ...
    *virtualenv* than your django site, you can provide a path to its
    ...
    def handle(self, *args, **options):
        ...
        for slug in args:
            try:
                project = SphinxDocProject.objects.get(slug=slug)
            except SphinxDocProject.DoesNotExist:
                raise CommandError('SphinxDocProject "%s" does not exist' % slug)
    ...
    def delete_documents(self, project):
        ...
        SphinxDoc.objects.filter(project=project).delete()

    def import_files(self, project):
        """
        Creates a :class:`~sphinxdoc.models.SphinxDoc` instance for each JSON
        file of ``project``.
        """
