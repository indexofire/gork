...
from gtag.models import TaggedItem, GenericTaggedItemBase
from gtag.utils import require_instance_manager
try:
    all
except NameError:
    # 2.4 compat
...
class TaggableManager(RelatedField):
    def __init__(self, verbose_name=_("Tags"), help_text=_("A comma-separated list of tags."), through=None, blank=False):
    ...
    def __get__(self, instance, model):
        if instance is not None and instance.pk is None:
            raise ValueError("%s objects need to have a primary key value before you can access their tags." % model.__name__)
        manager = _TaggableManager(through=self.through, model=model, instance=instance)
        return manager
